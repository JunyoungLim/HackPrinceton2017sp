...
for (i in c(1:(nrow(post2015)-1))) {
  cur_close <- cur_close * (1 + (post2015$pred[i]-0.5)/78);
...
for (i in c(1:(nrow(post2015)-1))) {
  cur_close <- cur_close * (1 + (post2015$pred[i]-0.5)/53);
...
for (i in c(1:(nrow(post2015)-1))) {
  cur_close <- cur_close * (1 + (post2015$pred[i]-0.5)/65);
...
# design
gg_plot <- gg_plot + ggtitle("FB Stock Prediction 2016-17") +
  ylab("stock prices") +
...
# design
gg_plot <- gg_plot + ggtitle("FB Stock 1994-2017") +
  ylab("stock prices") +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        plot.background = element_rect(fill = "white"),
        panel.background = element_rect(fill = "black"),
...
x <- ggplotly(gg_plot)
x
htmlwidgets::saveWidget(x, "FB prediction ensemble.html")
