...
# design
gg_plot <- gg_plot + ggtitle("COF Stock Prediction 2016-17 Ensemble") +
  ylab("stock prices") +
...
# design
gg_plot <- gg_plot + ggtitle("COF Stock 1994-2017") +
  ylab("stock prices") +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        plot.background = element_rect(fill = "white"),
        panel.background = element_rect(fill = "black"),
...
x <- ggplotly(gg_plot)
x
htmlwidgets::saveWidget(x, "COF prediction ensemble.html")
